...
export const useValidation = () => {
  ...
  const { snapshot, send } = useMachine(
    validationMachine.provide({
      actors: {
        ...
        validateSchema: fromPromise(
          async ({ input }: { input: any }): Promise<ErrorObject[] | null> => {
  ...
  const isValidating = computed(() => {
    const stateValue = snapshot.value.value;
    return (
      typeof stateValue === 'string' &&
      (stateValue.includes('parsing') ||
        stateValue.includes('validating') ||
        stateValue === 'bothReady') // bothReady immediately transitions to validatingInstance
    );
  });
