    ...
    <b-row class="mb-3">
      <b-col md="8" class="ml-auto mr-auto">
        <b-input-group>
          <b-input-group-text
            slot="prepend"
            :class="`${shareURL == null ? '' : 'text-success'}`"
          >
            Share schema
            <icon
          ...
          <b-input-group-append>
            <b-button variant="outline-dark" @click="copyLink()">
              ...
              <icon v-else :icon="['far', 'clipboard']" />
            </b-button>
          </b-input-group-append>
        </b-input-group>
      </b-col>
  ...
  methods: {
    ...
    copyLink: function() {
      ...
      this.$copyText(this.shareURL).then(
        ...
        function(e) {
          alert(
            'Cannot copy to clipbaord. Please select text and copy manually. Please report issue to @relequestual'
          );
    ...
    watchForDataChange: function() {
      const unwatch = this.$watch(
        function() {
          return `${this.schema}${this.instance}`;
        },
        function() {
